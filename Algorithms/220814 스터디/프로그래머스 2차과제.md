...
2-2. 다를 경우, 가능한 경우의 수는 number[right]와 동일한 수의 개수 ex) [-5, -2, 0, 2, 2, 3] => (-2,0,2), (-2, 
    ...
    for i in range(len(number) - 2):
        ...
        while left < right:
            ...
            # 2. 합한 수가 goal일 때 (target)
                ...
                # 정렬된 후에 둘이 같다면, 둘 사이에 있는 수는 모두 다 가능한 수 (ex. [-5 5 5 5 5])
                if number[left] == number[right]:
                    answer += right - left

                # 두 값이 다를 경우, (ex. [-5, -2, 0, 2, 3])
                else:
                    if max_idx > right:
                        max_idx = right

                        while max_idx >= 0 \
                        and number[right] == number[max_idx-1]:
                            max_idx -= 1
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MTE3NzA3OTksLTUxMzk2OTAzNiwxNz
Y1MzI5NDU2XX0=
-->